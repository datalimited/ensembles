...
#
# rocs <- filter(cv_sim_long, type == "mean") %>% group_by(method) %>%
#   do({get_roc(true = .$above1_true, est = .$above1_est)})
# ggplot(rocs, aes(spec, sens, colour = method)) + geom_line()
...
rocs <- cv_sim_binary_long %>% group_by(method) %>%
  do({get_roc(true = .$above_bbmsy1_true, est = .$bbmsy_est)})

p <- ggplot(rocs, aes(spec, sens, colour = method)) + geom_line() + coord_equal() +
